...
<Tabs>
  <Tab eventKey="pre_installation" title="Pre-Installation">
    ...
    Consider using the <a href="https://basc.pnnl.gov/hpwh_installation_tool" target="__new__">Heat Pump Water Heater Selection Tool </a> to help choose the optimal HPWH for the home.
...
| Air Volume = {props.data.volume_calculator?.length} x {props.data.volume_calculator?.width} x {props.data.volume_calculator?.height} = {props.data.volume_calculator?.length * props.data.volume_calculator?.width * props.data.volume_calculator?.height} ft<sup>3</sup> |
  ...
  <Tab eventKey="report" title="Report">
    ...
    *The photos and information in this report use timestamps and geotags to prevent the reuse of photos 
    ...
    ## Existing Condition
    <p>What is the fuel type of your existing water heater? <strong>{props.data?.existing_water_heater_fuel_type}</strong> </p>
    <p>What is the type of your water heating system before upgrading to a heat pump water heater? <strong>{props.data?.existing_water_heater_type}</strong> </p>
    <p>What is the storage capacity of the existing system? <strong>{props.data?.water_heater_storage_capacity}</strong> </p>
    ...
    <Photo id="old_nameplate_photo" label="Old Water Heater Nameplate – Photo">
